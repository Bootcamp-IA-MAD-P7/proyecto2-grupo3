...
class TestSalasRead:
    ...
    def test_get_salas_lista(self):
        ...
        assert data["items"][0]["nombre"] == "Test Sala"
...
class TestSalasPaginacionFiltros:

    def test_paginacion_default(self):
        for i in range(3):
            client.post("/api/salas/", json={
                "nombre": f"SalaPag{i}", "tematica": "Accion", "capacidad_max": 4, "precio": "30.00"
            })
        response = client.get("/api/salas/")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_filtro_nombre(self):
        client.post("/api/salas/", json={
            "nombre": "UnicaSala", "tematica": "Terror", "capacidad_max": 4, "precio": "25.00"
        })
        response = client.get("/api/salas/?nombre=UnicaSala")
        assert response.status_code == 200
        data = response.json()
        assert any(item["nombre"] == "UnicaSala" for item in data["items"])

    def test_filtro_tematica(self):
        client.post("/api/salas/", json={
            "nombre": "TematicaTest", "tematica": "Fantasía", "capacidad_max": 6, "precio": "35.00"
        })
        response = client.get("/api/salas/?tematica=Fantasía")
        assert response.status_code == 200
        data = response.json()
        assert any(item["tematica"] == "Fantasía" for item in data["items"])